...
let loop = function () {
  ...
  // Check if all particles are nearly still (low motion)
// 💖 Check speed to show birthday message
let totalSpeed = 0;
for (let i = 0; i < e.length; i++) {
  totalSpeed += Math.abs(e[i].vx) + Math.abs(e[i].vy);
}
...
ctx.fillStyle = "#ffffff";
  ...
  ctx.fillText("Happy Birthday", width / 2, height / 2 - 20);
  ctx.fillText("Ma Janan", width / 2, height / 2 + 20);
}
